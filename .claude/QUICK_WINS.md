...
### Deploy stops the daemon with a hard kill

deploy.ps1's stop step (including the elevated retry added in the commit titled "fix(deploy): stop the elevated daemon via an elevated retry") uses `Stop-Process`, although the daemon has a graceful path (`--stop` sets `TheCloserDaemonExitEvent`). Neither path preserves a pending timeout-repair record today: the daemon's exit path returns straight out of the watchdog loop, so the memory-mapped file, and any pending record in it, is dropped on graceful stop too, stranding the foreground lock timeout until reboot when the stop lands while a record is pending. Preferred shape, two halves: (1) the daemon's exit path runs one final `RepairIfCrashed` tick before returning (safe by construction: the repair only proceeds when `TheCloserGuardMutex` is acquirable, so a live app mid-suppression skips it); (2) deploy.ps1's stop invokes `TheCloser.Daemon.exe --stop` from the deploy target, then `Wait-Process` with a short timeout, falling back to `Stop-Process` only if the daemon does not exit. Elevation note: an unelevated `--stop` against an elevated daemon fails inside `EventWaitHandle.TryOpenExisting` with access denied (the event carries the elevated token's default DACL), so the graceful stop must also run through the elevated child.
