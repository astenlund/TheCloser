...
Reported: 2026-09-02. Fixed: 2026-09-02 in the commits titled "fix(shared): date the activation throttle from the press" (the incoming side) and "fix(shared): anchor the activation throttle on the previous press" (the stored side).
...
**Root cause:** two independent things coincided. The mouse emitted a double activation (second press 92 ms after the first; the daemon log shows the same ~100 ms signature routinely, normally skipped). Separately, the first close's SendInput blocked the daemon loop thread for about 2.1 s. That stall was priority starvation, not a slow hook: the elevated AutoHotkey logon task ran at Task Scheduler's default priority setting of 7 (Below Normal), the script and the daemon it launches inherited that class, and three agent CLIs were saturating every core with normal-priority work, so the daemon's thread was barely scheduled while SendInput waited to be resumed after each hook handoff. Reproduced under synthetic full-core load: a raw four-event SendInput inside the daemon took 1.8 s and 10.3 s, a Below Normal probe process took 9.9 s and 12.0 s cold with a 40 ms hot median, and a Normal probe process took 1.5 ms under the same load; without load every variant (elevated or not, Explorer or terminal foreground, Ctrl+W shape or Shift-up, raw or library, cold or hot) measured 1 to 2 ms. The resident keyboard hook owners (PowerToys Keyboard Manager and Command Palette, iCUE, the ChatGPT app) were cleared: their per-press cost is negligible, and Keyboard Manager off changed nothing. The throttle in `ActivationHandler` compared the handling time against the previous throttle tick, so when the queued second press was finally handled 2136 ms after the first tick write it passed, even though its payload QPC placed it 92 ms after the first press. The daemon IPC design (`bugs/intermittent-slow-invocation.md`) had accepted exactly this outcome, reasoning that a press collapsed behind a longer close and surviving the throttle honors a real double press; this fix supersedes that decision, since the payload timestamp can tell a double activation from a double press.

**Fix:** the throttle now subtracts the plausible press-to-handler latency from the current tick before comparing against the stored tick, so a press is judged by when the user pressed, not by when the loop reached it, and the same press-dated tick is what it stores, so the window is measured press-to-press and a genuine press shortly after a late handling is not dropped. The window is symmetric: the tick count's granularity is coarser than the QPC-derived latency, and an anchor written from handling time (an implausible payload, or the fallback app) can postdate a press that landed during that handling, so a small negative elapsed value is skipped too, while a stale-format or foreign tick still reads as a huge magnitude and stays unthrottled. When the payload is implausible the latency counts as zero and the throttle is equivalent to before: the only new skip case is a tick in the 200ms ahead of now, unreachable with the monotonic tick count both writers share. The standalone fallback app's throttle is unchanged: it is its own press.

**Stall fix:** the task is registered at priority 5 (Normal) through the shared `TheCloserTask.psm1` definition, `deploy.ps1` re-registers through the installer whenever any managed field of the live task differs from that definition, and the daemon and fallback app raise their own class to Normal at startup (`ProcessPriority.EnsureCurrentAtLeastNormal`, which logs and never throws, since the raise is optional and startup is not) as the backstop for a manual launch or a stale registration. Landed in the commit titled "fix(deploy): run the elevated task at normal priority". The same starvation explains the 100 to 490 ms press-to-handler latencies the daemon log recorded before it. Stall instrumentation (injection duration, handler phase breakdown, hook timeout at startup) landed in the commit titled "feat(shared): log stalled activation phases" and stays in place; the hook-timeout line remains informative because a genuinely unresponsive hook owner is still the other way an injection can stall.
