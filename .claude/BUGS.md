...
- Two Chrome invocations at 16:04. With Chrome itself foreground: rung 1 short-circuited (already foreground), no information. With explorer.exe foreground: both native rungs ran and failed honestly (`SetForegroundWindow returned false.` for the target and then the root; no AttachThreadInput failure line, so the target-thread attach succeeded), then the title bar click fallback engaged for the first time on record and succeeded. Net behavior: the correct tab closed both times. Usage note: the user hovers the *viewport*, not the tab strip, so CTRL-W closing the focused window's active tab is exactly the hovered tab; closing a specific background tab is out of scope for this bug and deliberately untracked: the app is bound to a mouse button, so Chrome's native middle-click-to-close is already an equivalent gesture away.
...
**Verification once fixed:** with explorer (or any non-Chrome app) focused, invoke TheCloser over a background Chrome window's *viewport* (the user's actual gesture; tab-strip hover is not a supported close gesture); that window's active tab should close. Verified 2026-07-11 16:27 for the base case: native activation of the root window succeeded with the owner attach in place, no click fallback, ~140ms. Remaining: repeat with Chrome minimized-then-restored, and with two Chrome windows where the hovered window is not Chrome's most recently used one (the closed tab must belong to the hovered window).
