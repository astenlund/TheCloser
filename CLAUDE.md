...
The deploy script stops the daemon, builds in Release mode, and copies the executables and invocation-layer files (`TheCloser.ahk`, `install-elevated-ahk.ps1`, `TheCloserTask.psm1`) to the deploy target configured in `deploy.settings.psd1` (git-ignored, machine-local; see `deploy.settings.example.psd1`). It then restarts the elevated AutoHotkey logon task, or re-registers it through the installer when the task is missing or any managed field of its live definition (paths, trigger, principal, priority, run limits) differs from the definition in `TheCloserTask.psm1`, the single source of truth both scripts import.
...
- Runs at Normal priority: the logon task is registered at Task Scheduler priority 5 because the default (7, Below Normal) is inherited by the script and by every process it launches, and a below-normal thread starves whenever normal-priority work saturates the cores (observed: one SendInput inside the daemon took 10 s under full-core load, 1.5 ms at Normal). `ProcessPriority.EnsureCurrentAtLeastNormal` at startup is the backstop for a manual launch or a stale task registration; it never lowers a deliberately higher class and never throws, since the raise is optional and startup is not
...
- On activation, consumes the press payload, logs press-to-handler latency, applies the shared 200ms throttle (dated from the press timestamp on both sides, so a double activation queued behind a stalled close is still skipped and a genuine press shortly after a late handling is not) and guard mutex, snapshots the current configuration, closes the window under the cursor, and dispatches the stuck-button healer when required. Stall attribution: a handling at or above `StallLogThresholdMs` (equal to the 200ms throttle window; a close measured 110 to 133 ms before the priority fix and should take about 65 ms after it) logs its settings/close phase breakdown, a keystroke injection at or above it logs its duration plus the current foreground process, a deferred press logs how long it queued behind the previous handling, and startup logs the `LowLevelHooksTimeout` registry value plus a line when the priority class had to be raised or could not be adjusted (SendInput resumes the caller after every low-level keyboard hook, so a starved caller or an unresponsive hook owner both stall injection; the resident hook owners were measured and cleared in the 2026-09-02 investigation, see `.claude/BUGS_HISTORY.md`)
...
- `ProcessPriority.cs`: `EnsureAtLeastNormal(Process)` raises a Below Normal or Idle class to Normal and reports whether it did; `EnsureCurrentAtLeastNormal(log)` is the startup form both entry points call, which logs the raise or the failure and never throws
...
### TheCloser.Tests
- xUnit tests cover the shared-memory protocol, activation handler, daemon runtime, watched and last-good configuration, repair protocol, close dispatch, foreground activation ladder, stuck-button healer, hook-timeout probe, process priority raise (on the test host's own process, restored afterwards), and logging
